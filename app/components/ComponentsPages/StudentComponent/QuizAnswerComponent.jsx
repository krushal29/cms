...
const QuizAnswerComponent = () => {
    ...
    const PrevShowHandle=()=>{
        setCurrentQuestion(currentQuestion-1);
    }

    const NextShowHandle=()=>{
        setCurrentQuestion(currentQuestion+1)
    }
    ...
    const filterQuestion=allQuestions.find((val,index)=>val.id==currentQuestion)
            ...
            <div className="w-[85%] text-center py-13">
                ...
                <div className="bg-white pb-5 rounded-2xl my-5">
                    <div className="bg-black rounded-tl-2xl rounded-tr-2xl ">
                        <h2 className="text-white font-bold text-2xl py-4">Answers</h2>
                    </div>


                    <div className="py-10  w-[75%] mx-auto">
                        <div className="bg-secondary border-2 border-black rounded-2xl my-5 py-5 px-8">
                            <h1 className="text-start font-medium text-2xl">Question {filterQuestion.id}</h1>
                            <div className="text-start py-8 border-b border-black">
                                <p className="font-medium  text-[18px]">{filterQuestion.id}. {filterQuestion.Question}</p>
                                <div className="space-y-5 pt-5">
                                    {filterQuestion.option.map((option,index)=>(
                                        <div className={`${true?'bg-green-200':'bg-rose-200'} w-[80%] rounded-[5px] py-2 px-5 flex items-center`} key={index}>
                                            {true? <IoMdCheckmark className='text-green-800'/>: <IoMdClose className='text-rose-800'/>}
                                       
                                        <p className="pl-2 font-medium text-[17px]">{option}</p>
                                    </div>
                                    ))}
                                </div>
                            </div>
                            <div className="flex justify-between py-5">
                                <Button className="text-[17px] py-5 px-8 cursor-pointer" onClick={PrevShowHandle} disabled={currentQuestion==1}>previous</Button>


                                {currentQuestion==QuestionLength?<Button className="text-[17px] py-5 px-8 cursor-pointer" onClick={()=>navigate('/QuizResult')}>Submit</Button>:<Button className="text-[17px] py-5 px-8 cursor-pointer" onClick={NextShowHandle}>Next</Button>} 
